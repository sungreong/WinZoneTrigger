...
- 시작 프로그램 등록과 트레이 실행
- 상단 `화면 갱신`으로 백그라운드 상태, 앱 감시 결과, 실행 로그를 수동 새로고침

## 최근 업데이트

- 상단 요약 바에 `화면 갱신` 버튼을 추가했습니다. 백그라운드 상태와 선택된 앱 감시의 최근 확인 결과를 즉시 다시 읽을 수 있습니다.
...
- `화면 갱신`: 백그라운드 상태, 선택된 앱 감시의 최근 확인 결과, 실행 로그를 지금 다시 읽습니다.
...
- 화면 표시가 늦어 보이면 상단 요약 바의 `화면 갱신`을 누르면 됩니다. 이 버튼은 자동 갱신 타이머를 기다리지 않고 백그라운드 상태, 선택된 앱 감시의 최근 확인 결과, 로그를 즉시 다시 읽습니다.
