...
- 시작 프로그램 등록과 트레이 실행
- 상태/로그 탭에서 조건 일치, 현재 위치, 보이는 Wi-Fi, 실행 로그 확인과 수동 새로고침
...
- `상태/로그`: 조건 일치 상태, 현재 좌표, 보이는 Wi-Fi, 최근 이벤트와 전체 로그를 확인합니다. `새로고침`으로 백그라운드 상태와 로그를 즉시 다시 읽을 수 있습니다.
...
- 화면 표시가 늦어 보이면 `상태/로그` 탭의 `새로고침`을 누르면 됩니다. 이 버튼은 자동 갱신 타이머를 기다리지 않고 백그라운드 상태와 로그를 즉시 다시 읽습니다.
